...
impl Browsercookies {
    ...
    pub fn from_browser(&mut self, b: Browser, domain_regex: &Regex) -> Result<(), Error> {
        match b {
            Browser::Firefox => firefox::load(&mut self.cj, domain_regex),
        }
    }

    pub fn to_header(&self, domain_regex: &Regex) -> Result<String, Error> {
...
mod tests {
    ...
    fn test_firefox() {
        ...
        if let Ok(cookie_header) = bc.to_header(&domain_regex) as Result<String, Error> {
